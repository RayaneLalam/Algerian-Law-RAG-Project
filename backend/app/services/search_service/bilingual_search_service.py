...
class BilingualSearchService:
    ...
    Supports Vector Search (default) and BM25 Search for both languages.
    """
    ...
    def __init__(self):
        ...
        self.french_bm25 = None  # New French BM25 attribute
        ...
        self.french_bm25_ready = False # Track French BM25
    ...
    def _load_indices(self):
        """Load FAISS indices, document collections, and BM25 for both languages."""
        self._load_french_index()
        self._load_french_bm25() # Added French BM25 loader
    ...
    def _load_french_bm25(self):
        """Load French BM25 index from pkl."""
        try:
            if os.path.exists(self.settings.FRENCH_BM25_PATH):
                ...
                logger.info("✓ French BM25 index loaded successfully")
            else:
                logger.warning(f"French BM25 path not found: {self.settings.FRENCH_BM25_PATH}")
        except Exception as e:
            logger.error(f"Failed to load French BM25: {e}")
    ...
    def _load_arabic_bm25(self):
        ...
        try:
            if os.path.exists(self.settings.ARABIC_BM25_PATH):
                ...
                logger.warning(f"Arabic BM25 path not found: {self.settings.ARABIC_BM25_PATH}")
        except Exception as e:
    ...
    def search(
        ...
        language: str = 'fr',
        top_k: int = 3,
        search_type: str = 'embeddings'
    ) -> List[Dict]:
        """
        Search documents in specified language using Vector or BM25 search.
        """
        ...
        if language == 'ar':
            ...
            # Mirror Arabic logic for French
            if search_type == 'bm25' and self.french_bm25_ready:
                logger.info("Using French BM25 search")
                results = self._search_french_bm25(query, top_k)
            elif self.french_ready:
                logger.info("Using French Vector search")
                results = self._search_french(query, top_k)
            else:
                logger.warning("French index not available, using multilingual fallback")
                results = self._search_multilingual(query, top_k, 'fr')
        
        return results

    def _search_french_bm25(self, query: str, top_k: int) -> List[Dict]:
        """French implementation of BM25 search."""
        try:
    ...
    # ... (Keep existing _search_french, _search_arabic, _map_arabic_doc, etc.)        
