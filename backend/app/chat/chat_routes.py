...
def chat_stream():
    try:
        ...
        chat_models.insert_message(conversation_id, role="user", content=message, sender_user_id=user_id)
        ...
        for i, res in enumerate(results, 1):
            # The service returns the doc fields at the top level
            art_id = res.get('article') or res.get('source', '?')
            content_snippet = res.get('content', '')[:100]
            sim = res.get('similarity', 0)
            
            current_app.logger.info(f"Result {i}: [{art_id}] (Sim: {sim:.3f}) - {content_snippet}")
